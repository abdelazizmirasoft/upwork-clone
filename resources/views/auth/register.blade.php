...
@section('content')
<h1 class="text-3xl text-green-500 mb-6 text-center">Complete your information</h1>
<form method="POST" action="{{ route('register') }}" class="w-full max-w-sm mx-auto rounded-lg border shadow-md p-5 mb-5">
    ...
    <div class="mb-4">
        ...
        <input id="name" type="text" name="name" class="shadow border rounded w-full p-2" value="{{ old('name') }}" autocomplete="name" placeholder="Username" autofocus>
       @error('name')
    ...
    <div class="mb-4">
        ...
        <input id="email" type="email" name="email" class="shadow border rounded w-full p-2" value="{{ old('email') }}" autocomplete="email" placeholder="Your email" autofocus>
        @error('email')
            <span class="text-red-400 text-sm">{{ $message }}</span>
        @enderror
    </div>
    ...
    <p for="role-select" class="font-semibold text-gray-700">I want to be :</p>
    <div class="flex justify-between items-center">
        <label for="freelance">Freelancer
    ...
    <button type="submit" class="bg-green-500 text-white hover:bg-green-700 transition ease-in-out duration-500 rounded-md shadow-md w-full block px-4 py-2 mt-3">Create my account</button>
</form>
@endsection