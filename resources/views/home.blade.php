...
@section('content')
  <h1 class="text-3xl text-green-500">Dashboard</h1>
  ...
  <h2 class="text-xl my-2"><svg class="w-6 h-6 inline-block" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      </svg> Your proposals ({{ auth()->user()->proposals->count() }})</h2>
    ...
    <span class="block font-semibold items-center">
       For the job "{{ $proposal->job->title }}"
    </span>
    <span>Cover letter : <span class="font-semibold">{{ $proposal->coverLetter->content }}</span></span>
    </div>
    @endforeach
  </section>
  ...
  <h2 class="text-xl my-2"><svg class="w-6 h-6 inline-block" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    </svg> Jobs you liked ({{ auth()->user()->likes()->count() }})</h2>
  ...
  <section class="text-sm text-gray-700 w-full">
    ...
    Your jobs ({{ auth()->user()->jobs()->count() }})</h2>
