<div>
    ...
    <h1 class="text-3xl text-green-500 mb-5">Job : {{ $conversation->job->title }}</h1>
    ...
    {{ $message->user->id === auth()->user()->id  ? 'bg-green-500 text-white mr-auto max-w-1/2 w-1/2' : 'ml-auto bg-gray-300 text-gray-700 max-w-1/2 w-1/2'}}">
        <p class="font-light">{{ $message->user->id === auth()->user()->id  ? 'You sent: ' : $message->user->name . ' He sent:'}}</p>
        <p>{{ $message->content }}</p>
    </div>
    @endforeach
